...
function safeGetItem(key, fallback = null) {
    ...
    catch (e) { console.warn('localStorage read error:', e); return fallback; }
}
...
function safeRemoveItem(key) {
    ...
    catch (e) { console.warn('localStorage remove error:', e); return false; }
}
function handleQuotaExceeded(key, value) {
    const errorMsg = 'Storage is full. Please export and clear old mindmaps.';
    console.error(errorMsg);
    const permit = confirm(errorMsg + ' Try to free space by removing the largest saved mindmap?');
    if (permit) {
        try {
            const indexData = safeGetItem('mindMapIndex');
            if (indexData) {
                const mindmaps = JSON.parse(indexData);
                if (mindmaps.length > 0) {
                    const removed = mindmaps.pop();
                    safeSetItem('mindMapIndex', JSON.stringify(mindmaps));
                    safeRemoveItem('mindmap_' + removed.name);
                    alert('Removed "' + removed.name + '" to free space. Please try saving again.');
                }
            }
...
function saveShortcutsValues() {
    for (let i = 1; i <= 51; i++) {
        ...
        safeSetItem(`modifier-${i}`, modifierValue);
        safeSetItem(`key-${i}`, keyValue);
    }
...
function loadShortcutsValues() {
    for (let i = 1; i <= 51; i++) {
        const modifierValue = safeGetItem(`modifier-${i}`);
        const keyValue = safeGetItem(`key-${i}`);
...
const addEventListeners = () => {
    ...
    document.getElementById('save-local').addEventListener('click', () => {
        const name = prompt('Save mindmap as:', mindmaptitle.innerText === 'Untitled Mindmap' ? '' : mindmaptitle.innerText);
        if (!name) return;
        mindmaptitle.innerText = name;
        autoSaveToLocalStorage();
        alert('Mindmap "' + name + '" saved to local storage.');
    });
...
// load description fonts from local system
const loadDescriptionFonts = () => {
    ...
    description_populateFontSelect(description_testFonts);
};
...
const drawLine = (circle1, circle2, labelText = '', lineStyle = 'solid', lineColor = '#00ff00', connectionType = 'normal') => {
...
// Auto-save mindmap data to localStorage for gallery recovery
function autoSaveToLocalStorage() {
    if (!mindmaptitle || mindmaptitle.innerText === 'Untitled Mindmap') return;
    const name = mindmaptitle.innerText;
    const mindMapData = {
        circles: circles.map(circle => ({
            id: circle.id, type: circle.type, text: circle.text.textContent,
    ...
    if (circles.length > 0 || lines.length > 0) {
        safeSetItem('mindmap_' + name, JSON.stringify(mindMapData));
        // Also update thumbnail (use neutral fills)
        if (svg && svg.children.length > 1) {
            const c = svg.cloneNode(true);
            c.querySelectorAll('circle, rect, polygon, ellipse').forEach(el => el.setAttribute('fill', '#0f0f1a'));
            c.querySelectorAll('text').forEach(el => el.setAttribute('fill', '#ffffff'));
            c.querySelectorAll('.collapseCircle').forEach(el => el.setAttribute('fill', '#ff6600'));
            c.querySelectorAll('.collapseCircleText').forEach(el => el.setAttribute('fill', '#ffffff'));
            const origin = c.querySelector('#origin');
            if (origin) origin.setAttribute('fill', '#ffffff');
            saveDataInIndex(name, new XMLSerializer().serializeToString(c));
        }
    }
};

// Auto-save on history changes (debounced)
...
// Shared function to build the mindmap from data (deduplicates LoadTemplate/loadFromJson/loadMindMapFromHistory)
function clearSVG() {
...
function applyCircleStyle(circleObj, data) {
    circleObj.description = data.description || '';
    circleObj.isCollapsed = data.isCollapsed || false;
        ...
        if (circle.isCollapsed) {
            ...
            const hideChildren = (parent) => {
                ...
                childCircles.forEach(child => {
                    ...
                    totalChildCount += 1 + hideChildren(child);
                });
                return totalChildCount;
            };
            circle.collapsetext.style.display = 'block';
            circle.collapse.style.display = 'block';
            let childCount = hideChildren(circle);
            circle.collapsetext.textContent = `${childCount}`;
        }
    });
}

function buildMindMapFromData(mindMapData) {
    // Create root circles (no parent)
    mindMapData.circles.forEach(data => {
        if (!data.parent) {
            createCircle(data.type, data.x, data.y, data.text, null, data.id);
            applyCircleStyle(circles[circles.length - 1], data);
        }
    });
    // Create child circles
    mindMapData.circles.forEach(data => {
        if (data.parent) {
            const parentObj = circles.find(c => c.id === data.parent);
            if (parentObj) {
                createCircle(data.type, data.x, data.y, data.text, parentObj.group, data.id);
                applyCircleStyle(circles[circles.length - 1], data);
            } else {
    ...
    mindMapData.connections.forEach(connection => {
        ...
        if (circleObj1 && circleObj2) {
            drawLine(circleObj1, circleObj2, connection.label, connection.type, connection.color, connection.connectiontype);
        } else {
...
const LoadTemplate = async function(path) {
    try {
        ...
        // Try localStorage first: extract name from path like "export/name.mndmp" or "MindmapData/name.mndmp"
        const nameMatch = path.match(/([^\/\\]+)\.mndmp$/);
        if (nameMatch) {
            const lsKey = 'mindmap_' + nameMatch[1];
            const lsData = safeGetItem(lsKey);
            if (lsData) mindMapData = safeParseJSON(lsData);
        }
        // Fall back to fetch for built-in templates
        if (!mindMapData) {
            const response = await fetch(path);
            if (!response.ok) throw new Error('Network response was not ok');
            mindMapData = JSON.parse(await response.text());
        }
        // Set title from path (e.g. "export/MyMap.mndmp" -> "MyMap", "MindmapData/SingleLine.mndmp" -> "SingleLine")
        const titleMatch = path.match(/([^\/\\]+)\.mndmp$/);
        if (titleMatch) {
            mindmaptitle.innerText = titleMatch[1];
        }
        history = [];
        clearSVG();
        buildMindMapFromData(mindMapData);
        clearHistory();
        if (mindMapData.history) {
            mindMapData.history.forEach(version => { history.push(version); });
        }
        restoreCollapsedStates();
        loadHistory();
        viewBox = { x: 0, y: 0, width: 600, height: 600 };
...
const loadFromJson = (file) => {
    ...
    reader.onload = (event) => {
        ...
        if (mindMapData.tasks) {
            mindMapData.tasks.forEach(task => { tasklistitems.push(task); });
        }
        restoreCollapsedStates();
        renderList();
        loadHistory();
...
function saveDataInIndex(name, svgData) {
    ...
    const oldmindMapData = raw ? safeParseJSON(raw, []) : [];
    const mindMapData = { 
        "name": `${name}`, 
    ...
    safeSetItem('mindMapIndex', JSON.stringify(oldmindMapData));
};
...
const saveAsJson = () => {
    ...
    // Clean thumbnail SVG: use neutral fills for consistent gallery display
    const thumbnailSvg = (function() {
        const c = svg.cloneNode(true);
        c.querySelectorAll('circle, rect, polygon, ellipse').forEach(el => el.setAttribute('fill', '#0f0f1a'));
        c.querySelectorAll('text').forEach(el => el.setAttribute('fill', '#ffffff'));
        c.querySelectorAll('.collapseCircle').forEach(el => el.setAttribute('fill', '#ff6600'));
    ...
    safeSetItem('mindmap_' + mindmaptitle.innerText, JSON.stringify({ ...mindMapData, _thumbnail: thumbnailSvg }));
    ...
    saveDataInIndex(mindmaptitle.innerText, thumbnailSvg);
    URL.revokeObjectURL(link.href);
};
...
const init = () => {
    theme = safeGetItem('mindMapTheme', 'dark');
    sHref = '';
    if (theme === 'light') {
